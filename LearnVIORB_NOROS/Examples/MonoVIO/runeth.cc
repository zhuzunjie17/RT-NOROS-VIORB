...
//     getline(inf,line);
    while (!inf.eof())
    {
        ...
        temp.timeStamp = (float)atof(line.substr(0,comma).c_str());
...
//     getline(inf,line);
	...
	string temp = line.substr(0,comma);
        imuTimeStamp = (float)atof(line.substr(0,comma).c_str());
...
int main(int argc, char **argv)
{
    ...
    //cv::waitKey(0);
    for(int j=0;j<iListData.size()-4;j=j+3)
    {
...
//                cout<<"*************************************************************************"<<endl;
  ...
  //              cout<<"=========================================================================="<<endl;
		...
		*/
	    ...
	    string temp = iListData[j].imgName.substr(0,iListData[j].imgName.size());
